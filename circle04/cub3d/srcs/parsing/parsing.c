...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/26 14:52:57 by zdevove           #+#    #+#             */
/*   Updated: 2023/06/26 15:37:45 by zdevove          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (!check_textures(data) && get_next_line(fd, &line, 0))
	{
		if ((size_t)ft_skip_spaces(line) == ft_strlen(line) || line[0] == '\n')
			free(line);
				...
				&& get_color(line + 2, data->color_roof))
			|| (!ft_strncmp(line, "F ", 2)
				&& get_color(line + 2, data->color_floor))
			...
			return (free(line), get_next_line(fd, &line, 1),
				ft_putendl_fd("Error\nMissing or invalid arguments", 2), 0);
	}
...
static int	get_map(t_data *data, char *line, int fd, int i)
{
	...
	while (get_next_line(fd, &line, 0))
	{
		...
		{
			data->map = realloc_map(data->map, sizeof(char *)
					* (i + 1), sizeof(char *) * (i + 2));
			if (!data->map)
				return (0);
